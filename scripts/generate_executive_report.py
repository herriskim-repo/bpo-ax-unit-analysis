...
def generate_markdown_report(risk_units: list[dict]) -> str:
    ...
    if not risk_units:
        report += "현재 KPI 초과 UNIT 없음.\n\n"
    else:
        report += f"KPI 초과 UNIT {len(risk_units)}개 발견. 운영 리스크 점검 필요.\n\n"

    report += "## KPI 초과 UNIT\n\n"

    if not risk_units:
        report += "- KPI 초과 UNIT 없음\n"
    else:
